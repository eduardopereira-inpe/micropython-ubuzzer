# conversor.py (RODE NO COMPUTADOR)

# Dicionário de frequências padrão
NOTES = {
...
def export_song_to_bytes(song_list):
    ...
    # 1. Record the time in 2 Bytes
    output.append((tempo >> 8) & 0xFF)
    output.append(tempo & 0xFF)
    
    # 2. Convert the notes (2 bytes of frequency + 1 byte of duration)
    for i in range(2, len(song_list), 2):
        note_name = song_list[i]
        duration = int(song_list[i+1])
        
        freq = 0 if note_name == "REST" else NOTES[note_name]
        
        output.append((freq >> 8) & 0xFF)
        output.append(freq & 0xFF)
        # Security offset for negative values
        output.append(duration + 64) 
